...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        // 1. Load claims file
        const claimsRes = await fetch("/claims.xlsx", { cache: "no-store" });
        if (!claimsRes.ok)
        ...
        try {
          const payRes = await fetch("/payments.xls", { cache: "no-store" });
          if (payRes.ok) {
  ...
  const cleanedClaims = React.useMemo(() => {
    return claims.map((c) => {
      ...
      if (
        ...
        nameLower.includes("michael kelly")
      ) {
        docSuffix = " - Chiro";
      } else if (
      ...
      const cleanDoc = docSuffix ? `${toTitleCase(cleanRaw)}${docSuffix}` : toTitleCase(cleanRaw);
